...
| `TRUSTED_PROXY_COUNT` | 앞단 신뢰 프록시 수. 로그인 기록·스로틀·데모 발급 제한이 X-Forwarded-For 를 오른쪽에서 이만큼 센 주소를 클라이언트로 본다. compose 가 기본값을 주므로 보통 비워 둔다. 로그인 잠금은 계정 단위라 이 값과 무관 | `prod.yml` 단독 `1`, `+proxied.yml` `2` |
